...
function AndroidDevice({
  ...
  statusBarBg, statusBarDark, screenBg, deviceBg,
}) {
  const baseBg = deviceBg || (dark ? '#1d1b20' : MD_C.surface);
  return (
    <div style={{
      ...
      <div style={{ background: statusBarBg || baseBg }}>
        <AndroidStatusBar dark={statusBarDark !== undefined ? statusBarDark : dark} />
      </div>
      {title !== undefined && <AndroidAppBar title={title} large={large} />}
      <div style={{ flex: 1, minHeight: 0, overflow: title !== undefined ? 'auto' : 'hidden', display: 'flex', background: screenBg || 'transparent' }}>
        {children}
      </div>
